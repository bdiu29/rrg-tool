...
- Route: `POST /api/rrg/backtest {timeframe, tail, exit:{model,...}, walk_forward}` → JSON report. Frontend: a "Strategy Backtest" tab on `rrg.html` (Plotly equity curve + histogram, per-call-type table, walk-forward fold table, Markdown export). Runs synchronously (daily walk-forward ≈ 4s).
