...
- **Signal-space vs display-space — the core decoupling (don't re-entangle these).** Two coordinate systems share the boundary at 100:
  - **Signal coords** (`sig_ratio`/`sig_mom`) drive *everything functional* — quadrant, phase, the EW squash, the gates and the call. Both intervals are σ-normalized by `_scale(series, window, center)`, which divides by a rolling std about a **fixed center** (RS-Ratio=100 where fast EMA==slow EMA; momentum=0) — **not** a rolling-mean z-score. Fixed-center is what makes the boundary honest and the units interval-independent (one gate set means the same on daily and weekly). The old weekly rolling-mean z-score silently meant "above its own recent average," not "fast EMA above slow" — that bug is gone.
...
- **Elliott-wave phase model (both intervals now):** counter-trend momentum is squashed at the boundary — `t_soft = tanh(Δtrend_len ratio / TREND_ETA)`; where `t_soft·mom < 0` (corrective leg: wave-2/4 pullback or wave-B bounce), `m_adj` is tanh-capped at `MOM_TAU` (raw-momentum units, applied *before* σ-normalization). Corrections approach the boundary but don't cross; only a genuine trend flip releases the cap. Each sector exports a `phase` field ("impulse ↑ (wave 3/5)" / "pullback (wave 2/4)" / "impulse ↓ (wave C/3)" / "bounce (wave B)" / "basing/topping (trend turn)") shown in the tooltip and feeding `_rotation_call` (bounce → WATCH not ROTATE IN; pullback → HOLD not ROTATE OUT).
...
- **Gate params live in `signal.DEFAULTS` and are threaded as a `params` arg — never mutated globally** (a backtest trial can't corrupt a concurrent live chart request). All in σ-units except `DIRECT_GATE` (scale-free): `DIRECT_GATE`, `MOVE_GATE`, `DIV_MARGIN`, plus the EW `TREND_ETA`/`MOM_TAU`. **Baked from a 4-fold walk-forward search** (`backtest.walk_forward_search`) — the combo with the best mean out-of-sample IN−OUT excess separation at +10d (≈+1.7%). Re-run that search to recalibrate against forward returns; this replaced the deleted image fit.
...
- `_rotation_call()` — decision logic: ROTATE IN / ROTATE OUT / HOLD / AVOID / WATCH. A leg counts as impulse (wave 3/5, can trigger ROTATE) only if `directness = net/path ≥ DIRECT_GATE` and `net ≥ MOVE_GATE` σ; bent/overlapping tails are corrective (wave 2/4) and never trigger a fresh ROTATE — the whipsaw guard. Momentum `DIV_MARGIN` σ below its tail peak while RS holds its high = wave-5 exhaustion → ROTATE OUT in Leading.
