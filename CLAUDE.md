...
**Current state:** Home hub (live status badges) + RRG tool (sector rotation) + Schwab account module (positions with sector signals) + Breadth module (market breadth tracker with regime filter) + Screener module (TradingView-style filters, watchlists, intraday pump/dump alerts with Discord/email routing, EMA/golden-pocket filters, and a strategy backtester sidecar).
